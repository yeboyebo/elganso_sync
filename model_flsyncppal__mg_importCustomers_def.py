...
class elganso_sync(interna):
    ...
    @transaction.atomic
    def elganso_sync_getUnsynchronizedCustomers(self):
        _i = self.iface
        ...
        try:
            ...
            if qsatype.FLUtil.isInProd():
                ...
                url = 'http://local2.elganso.com/syncapi/index.php/customers/unsynchronized'
            ...
            print(e)
            syncppal.iface.log("Error. No se pudo establecer la conexión con el servidor.", 'mgsynccust')
            return cdLarge
        ...
        if json and len(json):
            try:
                ...
                if not aCustomers and not isinstance(aCustomers, (list, tuple)):
                    syncppal.iface.log("Error. Ocurrió un error al sincronizar los clientes.", 'mgsynccust')
                    return cdSmall
            except Exception as e:
                print(e)
                return cdSmall
            ...
            if aCustomers and len(aCustomers):
                syncppal.iface.log(ustr("Éxito. Los siguientes clientes se han sincronizado correctamente: ", ustr(aCustomers)), 'mgsynccust')
                for customer in aCustomers:
                    try:
                        url = None
                        if qsatype.FLUtil.isInProd():
                            url = 'https://www.elganso.com/syncapi/index.php/customers/' + str(customer) + '/synchronized'
                        else:
                            url = 'http://local2.elganso.com/syncapi/index.php/customers/' + str(customer) + '/synchronized'

                        print("Llamando a", url)
                        response = requests.put(url, headers=headers)
                        print("Correcto")
                    except Exception:
                        syncppal.iface.log(ustr("Error. El cliente ", customer, " no ha podido marcarse como sincronizado."), 'mgsynccust')
            elif aCustomers == []:
                syncppal.iface.log("Éxito. No hay clientes que sincronizar.", 'mgsynccust')
                return cdLarge
        else:
            syncppal.iface.log("Éxito. No hay clientes que sincronizar.", 'mgsynccust')
            return cdLarge
    ...
    def elganso_sync_processCustomers(self, customers):
        try:
            aCustomers = []
            for customer in customers:
                ...
                if not curTab.commitBuffer():
                    syncppal.iface.log(ustr("Error. No se pudo guardar el cliente ", str(email)), 'mgsynccust')
                    return False
    ...
    def getUnsynchronizedCustomers(self):
        return self.ctx.elganso_sync_getUnsynchronizedCustomers()

    def processCustomers(self, customers):
        return self.ctx.elganso_sync_processCustomers(customers)
