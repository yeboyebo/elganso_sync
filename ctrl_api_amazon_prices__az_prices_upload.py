...
class AzPricesUpload(AzFeedsUpload, ABC):
    ...
    def get_query(self):
        ...
        q.setFrom("az_articulosamazon az INNER JOIN articulos a ON az.referencia = a.referencia INNER JOIN atributosarticulos aa ON az.referencia = aa.referencia LEFT OUTER JOIN articulostarifas at ON (a.referencia = at.referencia AND at.codtarifa = (SELECT valor FROM param_parametros WHERE nombre = 'TARIFA_AMAZON')) LEFT OUTER JOIN eg_articulosplan app ON (a.referencia = app.referencia AND app.codplan IN (SELECT pp.codplan FROM eg_planprecios pp WHERE pp.vigente AND CURRENT_DATE BETWEEN pp.desde AND pp.hasta AND pp.codplan IN (SELECT codplan FROM eg_tiendasplanprecios WHERE codtienda = 'AMAZ') ORDER BY hasta DESC LIMIT 1)) LEFT OUTER JOIN eg_planprecios pp ON app.codplan = pp.codplan")
        q.setWhere("az.referencia IN (SELECT referencia FROM az_articulosamazon WHERE sincroarticulo AND articulocreado AND sincroimagenes AND sincrorelacion AND NOT sincroprecio AND NOT errorsincro LIMIT 15) GROUP BY az.referencia, aa.barcode, at.pvp, a.pvp, app.pvp, pp.desde, pp.hasta, pp.horadesde, pp.horahasta")
