...
class AzPricesUpload(AzFeedsUpload, ABC):
    ...
    def get_query(self):
        ...
        q.setFrom("az_articulosamazon az INNER JOIN articulos a ON az.referencia = a.referencia INNER JOIN atributosarticulos aa ON az.referencia = aa.referencia LEFT OUTER JOIN articulostarifas at ON (a.referencia = at.referencia AND at.codtarifa = (SELECT valor FROM param_parametros WHERE nombre = 'TARIFA_AMAZON')) LEFT OUTER JOIN eg_articulosplan ap ON (a.referencia = ap.referencia AND ap.codplan in (SELECT pp2.codplan FROM eg_planprecios pp2 INNER JOIN eg_tiendasplanprecios tp2 on (pp2.codplan = tp2.codplan AND tp2.codtienda = 'AMAZ') INNER JOIN eg_articulosplan ap2 on (pp2.codplan = ap2.codplan AND a.referencia = ap2.referencia) WHERE pp2.hasta >= CURRENT_DATE AND pp2.vigente ORDER BY pp2.desde DESC, pp2.hasta ASC LIMIT 1)) LEFT OUTER JOIN eg_planprecios pp ON ap.codplan = pp.codplan")
