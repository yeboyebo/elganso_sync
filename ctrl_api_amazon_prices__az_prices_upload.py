...
class AzPricesUpload(AzFeedsUpload, ABC):
    ...
    def get_query(self):
        ...
        q.setWhere("az.referencia IN (SELECT referencia FROM az_articulosamazon WHERE sincroarticulo AND articulocreado AND sincroimagenes AND sincrorelacion AND NOT sincroprecio AND NOT errorsincro LIMIT {}) AND barcode not in (select barcode from atributosarticulos where a.referencia = aa.referencia and aa.talla >= '35' and (a.codgrupomoda = '3' OR a.codgrupomoda = '5')) GROUP BY az.referencia, aa.barcode, at.pvp, a.pvp, app.pvp, pp.desde, pp.hasta, pp.horadesde, pp.horahasta".format(self.driver.azQueryLimit))
