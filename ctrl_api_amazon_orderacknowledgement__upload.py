...
class elganso_sync_upload(interna_upload):
    ...
    def start(pk, data):
        ...
        if "passwd" in data and data["passwd"] == syncppal.iface.upload_param_sincro('apipass')['auth']:
            response = task_manager.task_executer("amazon_orderacknowledgement_upload", data)
