...
from controllers.api.amazon.returns.serializers.movistock_serializer import AzMovistockSerializer
...
class ReturnLineSerializer(DefaultSerializer):
    ...
    def get_data(self):
        ...
        if not qL.exec_():
            syncppal.iface.log("Error. Falló la query al obtener los datos de las líneas de la venta original", "azreturnsget")
            return False

        if not qL.first():
            syncppal.iface.log("Error. No se encontraron datos de las líneas de la venta original", "azreturnsget")
            return False
        ...
        movistock_data = AzMovistockSerializer().serialize(new_data)
    ...
    def get_barcode(self):
        return self.get_init_value("merchant_sku")
    ...
    def get_cantidad(self):
        return float(self.get_init_value("return_quantity")) * -1 or 0
