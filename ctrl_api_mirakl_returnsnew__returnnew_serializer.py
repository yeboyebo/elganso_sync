...
from controllers.api.mirakl.returnsnew.serializers.return_line_serializer import ReturnLineSerializer
...
class ReturnNewSerializer(DefaultSerializer):
    ...
    def get_data(self):
        ...
        bodyMensaje = str(self.get_init_value("datosdevol"))
        bodyMensaje = bodyMensaje.replace("\"", "__aqcomillas__")
        bodyMensaje = bodyMensaje.replace("'", "\"")
        bodyMensaje = bodyMensaje.replace("__aqcomillas__", "'")
        mensajeDevol = json.loads(bodyMensaje)

        self.set_string_value("direccion", str(mensajeDevol["message"]["collection_address"]), max_characters=100)
        ...
        total = 0
        neto = 0
        for item in mensajeDevol["message"]["returns"]:
            if "ean" not in item:
                return False
            item.update({
